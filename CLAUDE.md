...
6. **Per-Hetzner-project isolation.** A dedicated `vm-servers` Hetzner project (originally created as `revit-vm`; user may rename in the Hetzner console at their leisure — it's cosmetic), separate from other projects on the same account. Standing pattern: "Cloud project per repo, fnox item per repo". Blast radius — a script bug or token leak only affects this project's resources.
...
14. **Multi-provider: Hetzner (default) + Vultr Bare Metal (opt-in).** Provider is selected via `VM_PROVIDER` env var in `mise.local.toml` — `"hetzner"` (default) or `"vultr"`. Daily-use tasks (`start`/`stop`/`push`/`pull`) and shared helpers (`vm:up`/`vm:down`/`vm:ip`/`vm:status`/`snapshot:list`/`debug:*`) all dispatch on this. Hetzner-only paths (`vm:up-qemu`, `vm:up-kvm`, `snapshot:create`, `debug:metrics`) keep their explicit names. Vultr-only paths live under the `vultr:*` namespace (`vultr:up`, `vultr:down`, `vultr:status`, `vultr:token:set`). Rationale: Vultr Bare Metal is the only cheap hourly KVM host that bridges cpx42-TCG (slow daily driver) and AX41 (monthly KVM lock-in); supporting it as an alternative beats forcing a choice. Per-provider tokens via fnox (`HCLOUD_TOKEN`, `VULTR_API_KEY`).

15. **Vultr snapshot pipeline — Cloudflare R2 as transit bucket.** Vultr BM has no hot-snapshot API; `vultr-cli snapshot create --id <vm>` is Cloud Compute (VPS) only. The wired pipeline ([providers/vultr/snapshot-create.nu](providers/vultr/snapshot-create.nu)): (a) ssh in, ensure awscli is installed, (b) `docker stop --timeout=120`, (c) `gzip -1 < /root/windows_storage/*.qcow2 | aws --endpoint-url $R2_ENDPOINT s3 cp - s3://$R2_BUCKET/...` — credentials inlined per-call, (d) Mac-side `aws s3 presign` for a 24h URL, (e) `vultr-cli snapshot create-url --url <presigned>`, (f) poll until status=complete (loop, 30s ticks, 60min cap), (g) `aws s3 rm` to drop the R2 transit object — Vultr has its own copy now. R2 chosen over Vultr Object Storage because (i) free egress so Vultr's fetch costs nothing, (ii) provider-neutral — same bucket enables future cross-provider transfers (Hetzner → Vultr or vice versa), (iii) already in the CF ecosystem this repo lives in. Snapshots in Vultr are filtered by description prefix `vm-servers-*` so prune/list don't touch unrelated snapshots in the same account. Expect 30-60 min per `stop` on Vultr (~40 GB upload over WAN).

16. **Provider switch lives in `mise.local.toml`, never in `mise.toml`.** `mise.toml [env]` defaults `VM_PROVIDER = "hetzner"` so the existing Hetzner UX is preserved with zero local-toml setup. Switching providers is a per-checkout decision (different repo clone = different provider) and must not bleed across machines. If you find yourself wanting to change the default in `mise.toml`, you almost certainly want a local override instead.
...
- **Per-stage subfolder `mise.toml` files (e.g. `connect/mise.toml`, `debug/mise.toml`) as scrapers-catalogs does.** Tested live 2026-05-22. mise's hierarchical config loads parent configs up the tree, NOT descendants — `mise tasks ls` from the repo root does NOT see tasks defined in subfolder configs. The fix is `experimental_monorepo_root = true`, but that re-namespaces tasks as `//connect:rdp:wait` (requires `MISE_EXPERIMENTAL=1`) and breaks the documented 4-command UX. Decision: ALL task defs live in the root `mise.toml`; encapsulation comes from `.nu` script files in stage folders (`connect/*.nu`, `debug/*.nu`, `providers/{hetzner,vultr}/*.nu`). The root `mise.toml` is a thin task-surface table whose bodies are one-liners pointing at `.nu` files. Don't reintroduce subfolder `mise.toml`s — the rejected experiment is preserved in git history.
...
- Don't change the `VM_PROVIDER` default in `mise.toml [env]` from `"hetzner"`. Switching providers is a per-checkout decision via `mise.local.toml`; flipping the committed default would silently change UX for every other clone of this repo.
- Don't bypass the R2 transit step on Vultr. Vultr BM has no native hot-snapshot — `snapshot:create` on Vultr MUST go through the R2 pipeline ([providers/vultr/snapshot-create.nu](providers/vultr/snapshot-create.nu)). If R2 isn't configured, the script errors clearly pointing at `r2:bootstrap`; do not add a "skip snapshot and just destroy" fallback. See decision 15.
