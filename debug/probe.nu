...
print $"=== VM: ($ip) ==="
print "--- container ---"
^ssh -i $key -o StrictHostKeyChecking=accept-new $"root@($ip)" 'bash -c "docker ps -a; echo \"restart policy: $(docker inspect windows --format {{.HostConfig.RestartPolicy.Name}} 2>/dev/null)\"; echo \"mounts:\"; docker inspect windows --format \"{{range .Mounts}}  {{.Source}} -> {{.Destination}}{{println}}{{end}}\" 2>/dev/null"'
